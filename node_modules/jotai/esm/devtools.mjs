...
const isEqualAtomsValues$1 = (left, right) => left.size === right.size && Array.from(left).every(([left2, v]) => Object.is(right.get(left2), v));
const isEqualAtomsDependents$1 = (left, right) => left.size === right.size && Array.from(left).every(([a, dLeft]) => {
...
function useAtomsSnapshot(scope) {
  ...
  useEffect(() => {
    ...
    const callback = () => {
      ...
      if (isEqualAtomsValues$1(prevValues, values) && isEqualAtomsDependents$1(prevDependents, dependents)) {
        return;
      }
      prevValues = values;
